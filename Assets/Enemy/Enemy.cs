...
public abstract class Enemy : MonoBehaviour
{
    ...
    {
        float toMult = 1 + (LevelInfo.level / 2f); // Scaling factor based on the game's level
        chilled = 1;
        m_maxHealth *= toMult; // Scale the max health
        m_attackDamage *= toMult; // Scale the attack damage
        CurrHealth = MaxHealth;
        Instantiate(SpawnEffect, gameObject.transform.position, Quaternion.identity); // Instantiate a spawn effect
    ...
    public Vector2 Position => transform.position; // Get the position of the enemy
    public float MaxHealth => m_maxHealth; // Get the maximum health of the enemy
    public float AttackDamage => m_attackDamage; // Get the attack damage of the enemy
    abstract public bool IsAttacking { get; } // Abstract property indicating if the enemy is attacking
    public float lastTimeBeHit { get; protected set; } // Time when the enemy was last hit
    public float chilled { get; protected set; } // Chilled state: 1 = normal, 0.5 = slowed, 0 = frozen
    #endregion

    #region Public Methods
    /**
     * Coroutine that lets the enemy sprite to flash in color for a duration in seconds
     */
